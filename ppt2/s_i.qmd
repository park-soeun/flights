...
- 데이터 병합 및 컬럼 처리
    - `flights.csv`와 `planes.csv`를 `tailnum` 기준으로 병합
```{python}
#| echo: true
df = pd.merge(flights, plane_df, on='tailnum')
```
...
<br>
    ...
    - `datetime` 컬럼 생성 (`year`, `month`, `day`, `hour`, `minute`를 변환)
```{python}
#| echo: true
...
- 만석에 가까울수록 탑승 절차, 수하물 처리 등이 길어져 지연될 가능성이 있음
...
# 출발 지연이 가장 긴 모델 TOP 10 (Bad Flights)
sns.barplot(ax=axes[0], x=delay_time_top['manu_model'], y=delay_time_top['mean'], palette="Reds_r")
axes[0].set_ylim(0, 50)
axes[0].set_xlabel("Model by Manufacturer", fontsize=14)
axes[0].set_ylabel("Average Delay Time (min)", fontsize=14)
axes[0].set_title("Top 10 Most Delayed Aircraft Models", fontsize=16)
axes[0].tick_params(axis='x', rotation=40)

# 출발 지연이 가장 짧은 모델 TOP 10 (Good Flights)
sns.barplot(ax=axes[1], x=delay_time_bottom['manu_model'], y=delay_time_bottom['mean'], palette="Blues_r")
axes[1].set_ylim(-5, 10)
axes[1].set_xlabel("Model by Manufacturer", fontsize=14)
axes[1].set_ylabel("Average Delay Time (min)", fontsize=14)
axes[1].set_title("Top 10 Least Delayed Aircraft Models", fontsize=16)
